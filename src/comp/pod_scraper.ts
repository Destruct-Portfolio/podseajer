  ...
  async $extract() {
    ...
    if (this.$page !== null) {
      ...
      await this.$page.type(
        "#user_password",
        "Mdo2@8nv3H!i4BbYWjhHogJxj2Zkqn",
        { delay: 100 }
      );
      ...
      await this.$page.click(
        "#kt_content > div > div.podseeker-container.container.search-result > div.row.advanced-search > div.col-sm-12 > a",
        { delay: 100 }
      );
      await Delay(5000);
      await this.$page.click(
        "#podcast_search > div:nth-child(1) > div:nth-child(1) > div > div > div > span > span.selection > span > ul > li > input",
        { delay: 100 }
      );
      await this.$page.waitForSelector(
        "#select2-q_podcast_genres_genre_id_in-results>li",
        { timeout: 30 * 1000 * 1 }
      );
      let categories = fs
      ...
      for (var i = 0; i < categories.length; i++) {
        ...
        if (!categorie.includes(">")) continue;
        await this.$page.type("#podcast_search", categorie);
        await this.$page.click(
          "#kt_content > div > div.podseeker-container.container.search-result > div:nth-child(1) > div > div > button"
        );
        await Delay(5000);
        ...
        let results = await this.$page.evaluate(() => {
          return (
            document.querySelector("#result_query_pod > span") as HTMLElement
          ).innerText;
        });
        ...
        while (total_scraped !== parseInt(results.split(" ")[0])) {
          // scrape the pods
          let x_d = await this.$page.evaluate(
            async (x) => {
              let podcasts: any = [];
              let records = Array.from(
                document.querySelectorAll("div.podcast-info")
              );
              ...
              for (let i = 0; i < records.length; i++) {
                try {
                  ...
                  let tem = {
                    name: (
                      elemen.querySelector(
                        "div.row > div > div.text-truncate"
                      ) as HTMLElement
                    ).innerText,
                    img: (elemen.querySelector("img") as HTMLImageElement).src,
                    genera: (
                      elemen.querySelector(
                    ...
                    active: (
                      elemen.querySelector(
                        "div.row > div:nth-child(6)"
                      ) as HTMLElement
                    ).innerText.split("\n")[0],

                    link: (elemen.querySelector("a") as HTMLAnchorElement).href,
                  };
                  console.log(tem);
                  podcasts.push(tem);
                } catch (err) {
          ...
          for (let index = 0; index < x_d.length; index++) {
            let ele = x_d[index];
            let fd = await this.$page.evaluate(
              async (x) => {
                console.log(x.x);
                let email = x.x;
                async function displayFileContentsPromise(x: string) {
                  return fetch(x, {
                    headers: {
                      accept:
                      ...
                      newrelic:
                        "eyJ2IjpbMCwxXSwiZCI6eyJ0eSI6IkJyb3dzZXIiLCJhYyI6IjM3MDcwNjYiLCJhcCI6IjUzNTg5OTEzMiIsImlkIjoiZWE4YzVjMGRkN2ViMDllNiIsInRyIjoiOThhMDAzZjc3Zjg5MDg2NzRiNGM4MTk4YWE1MTZjMDAiLCJ0aSI6MTY5Nzc0MjU4OTI3N319",
                      "sec-ch-ua":
                      ...
                      "sec-gpc": "1",
                      traceparent:
                        "00-98a003f77f8908674b4c8198aa516c00-ea8c5c0dd7eb09e6-01",
                      tracestate:
                        "3707066@nr=0-1-3707066-535899132-ea8c5c0dd7eb09e6----1697742589277",
                      "x-csrf-token":
                        "pgolJUscpL9kaUgEN9OtUEvD0+Mx4WnO2TLREV8JCTJDIbUIjEltX2iyQp5SGFzKHhKCTtDWIg68nuvv7tJuQA==",
                ...
                let xdr: any = await displayFileContentsPromise(email);
          ...
          await this.$page!.evaluate(() => {
            (
              document.querySelector("#load-more-podcasts") as HTMLAnchorElement
            ).click();
          });
